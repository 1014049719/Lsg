...
public class CommonAlertDialog implements View.OnClickListener {
	...
	View mDelimitMsg, mDelimitBtn;

	public CommonAlertDialog(Context context) {
		mAlertDlg = new Dialog(context, R.style.AlertDialog);
		mAlertDlg.setContentView(R.layout.dialog_alert_comment);
		...
		mDelimitBtn = (View) window.findViewById(R.id.v_delimit_btn);
	...
	public void show() {
		// NOTE(lijianfeng)
		// 当activity已经结束，Dialog.show()会抛出WindowManager.BadTokenException异常
		try {
			mAlertDlg.show();
			Window window = mAlertDlg.getWindow();
			WindowManager.LayoutParams params = window.getAttributes();
