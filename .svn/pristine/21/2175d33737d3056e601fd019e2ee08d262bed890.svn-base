...
public class CameraLauncher {
	...
	public int getWidthLimit() {
		return mWidthLimit > 0 ? mWidthLimit : maxCaptureImageSize;
	}
	...
	public int getHeightLimit() {
		return mHeightLimit > 0 ? mHeightLimit : maxCaptureImageSize;
	}
	...
	private FileObserver mCameraRollObserver = new FileObserver(Environment.DIRECTORY_DCIM, FileObserver.CREATE) {
		...
		public void onEvent(int event, String path) {
			...
			AppLogger.d("Detected a image file captured by camera", path);
		}
	};

	public CameraLauncher(Activity activity, int resultCode, CameraLauncherListener listener) {
		mActivity = activity;
		mResultCode = resultCode;
		mCameraLauncherListener = listener;
	}

	public CameraLauncher(Fragment fragment, int resultCode, CameraLauncherListener listener) {
		mFragment = fragment;
	...
	public void onCameraCancel() {
		if (Checker.isExistedFile(mPhotoFile))
			mPhotoFile.delete();
	}
	...
	public File onCameraReturn() {
		...
		if (Checker.isEmpty(mPhotoFile)) {
			AppLogger.w("Cannot found captured image file, try to load from new added files (size=%s).",
		...
		if (Checker.isEmpty(mPhotoFile)) {
			AppLogger.e("Failed to found photo file captured by camera.");
		...
		if (mOutputFile != null && getWidthLimit() > 0 && getHeightLimit() > 0) {
			...
			if (imageSize.width > getWidthLimit() || imageSize.height > getHeightLimit()) {
				...
				Bitmap documentedBitmap = BitmapUtil.loadBitmapFromFile(mPhotoFile, getWidthLimit(), getHeightLimit(),
				...
				FileUtil.copyFile(mPhotoFile, mOutputFile);

			if (Checker.isEmpty(mOutputFile)) {
				AppLogger.e("Failed to save documented photo file %s.", mOutputFile);
				return null;
			}
